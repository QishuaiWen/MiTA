...
class MiTA_Attention_Conv(nn.Module):
    def __init__(
            self,
            dim: int,
            num_heads: int = 8,
        ...
        self.kv_topk = self.num_expert + 5
    ...
    def mita(self, query, key, value, router, router_topk=1, kv_topk=16):
        ...
        # temp debug
        # print('moba_q.shape, moba_kv.shape, moba_cu_seqlen_q.shape, moba_cu_seqlen_kv.shape, moba_q_sh_indices.shape')
        # print(moba_q.shape, moba_kv.shape, moba_cu_seqlen_q.shape, moba_cu_seqlen_kv.shape, moba_q_sh_indices.shape)
        # print(moba_cu_seqlen_q)
        # assert 0 not in moba_seqlen_q
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # x.shape e.g., [256, 201, 384], 1 cls token, 196 image tokens, 4 register tokens
        # print('using mita attention with dwc!')
        B, N, C = x.shape
        ...
        H = W = int((N - 5) ** 0.5)
        q = q.reshape(B, N, -1)
        router = self.pool(q[:, 1:-4, :].reshape(B, H, W, C).permute(0, 3, 1, 2)).reshape(B, C, -1).permute(0, 2, 1)
        # router = torch.cat((q[:, :1, :], router, q[:, -4:, :]), dim=1)
        ...
        # raise Exception(q.shape, k.shape, v.shape, router.shape)

        # dwc
        v_ = v[:, :, 1:-4, :].transpose(1, 2).reshape(B, H, W, C).permute(0, 3, 1, 2).contiguous()
        x_ = self.dwc(v_).permute(0, 2, 3, 1).reshape(B, N - 5, C)
        
        x = self.mita(q, k, v, router, router_topk=1, kv_topk=self.kv_topk)
        x = x.transpose(1, 2).reshape(B, N, C)

        x = x + F.pad(x_, (0, 0, 1, 4))
        # assert torch.equal(F.pad(x_, (0, 0, 1, 4))[:, 1:-4], x_)

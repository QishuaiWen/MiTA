...
class MiTA_Attention(nn.Module):
    def __init__(
        ...
        # dwc
        # self.gate = nn.Parameter(torch.zeros(1, 1024, 192))
        # self.g = nn.Parameter(torch.zeros(dim, dim))
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        ...
        # dwc
        # v_ = v[:, :, 1:, :].transpose(1, 2).reshape(B, H, W, C).permute(0, 3, 1, 2).contiguous()
        # x_ = self.dwc(v_).permute(0, 2, 3, 1).reshape(B, N - 1, C)
        
        if q.dtype == torch.float32:
            q, k, v, router = q.half(), k.half(), v.half(), router.half()
        
        # print(router.shape, self.num_expert, self.kv_topk)

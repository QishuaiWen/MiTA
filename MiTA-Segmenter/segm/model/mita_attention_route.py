...
class MiTA_Attention_Route(nn.Module):
    def __init__(
            self,
            dim: int,
            num_heads: int = 8,
        ...
        # averge pooling
        self.pool_size = 9  # 5, 7, 9
        self.num_expert = self.pool_size ** 2
        self.kv_topk = self.num_expert
    ...
    def mita(self, query, key, value, router, router_topk=1, kv_topk=16):
        ...
        # moba attn        
        moba_attn_out = flash_attn_varlen_func(  # as router_topk=1, no online softmax
            q=moba_q,
            k=moba_kv[:, 0],
            v=moba_kv[:, 1],
            cu_seqlens_q=moba_cu_seqlen_q,
            cu_seqlens_k=moba_cu_seqlen_kv,
            max_seqlen_q=moba_seqlen_q.max().item(),
            max_seqlen_k=kv_topk,
            softmax_scale=d ** (-0.5),
            causal=False,
            dropout_p=0.0,
        )  # [B*N*H, 1, d]

        output = torch.zeros((B*N*H, d), device=query.device, dtype=torch.float16)  # [B*N*H, d]
        output.index_add_(0, moba_q_sh_indices, moba_attn_out.squeeze(1))
        # output = output.to(query.dtype)

        return rearrange(output, '(B N H) d -> B H N d', B=B, N=N)

    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # print('using route-only mita attention')
        B, N, C = x.shape
        qkv = self.qkv(x).reshape(B, N, 3, C).permute(2, 0, 1, 3)  # [3, B ,N, C]
        q, k, v = qkv.unbind(0)  # [B, N, C]
        
        if N == 1024 + 1 or N == 1600 + 1:
            H = W = int((N - 1) ** 0.5)
        ...
        elif N == 1024 + 150 or N == 1600 + 150:
            print('using mita attention in the decoder!')
            H = W = int((N - 150) ** 0.5)
            router = self.pool(q[:, :-150, :].reshape(B, H, W, C).permute(0, 3, 1, 2)).reshape(B, C, -1).permute(0, 2, 1)
        else:
            raise Exception(N)
        ...
        if q.dtype == torch.float32:
            q, k, v, router = q.half(), k.half(), v.half(), router.half()
        
        # print(q.shape, router.shape, self.kv_topk)
        x = self.mita(q, k, v, router, router_topk=1, kv_topk=self.kv_topk * 2)

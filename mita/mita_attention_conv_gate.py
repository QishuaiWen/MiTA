...
class MiTA_Attention_Conv_Gate(nn.Module):
    def __init__(
            self,
            dim: int,
            num_heads: int = 8,
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        # print('using mita attention with conv')
        B, N, C = x.shape
        # qkv = self.qkv(x).reshape(B, N, 3, C).permute(2, 0, 1, 3)  # [3, B ,N, C]
        qkvg = self.qkvg(x).reshape(B, N, 4, C).permute(2, 0, 1, 3)
        q, k, v, g = qkv.unbind(0)  # [B, N, C]
